...
BigInt_add:
    ...
    else2:
        ...
        // ARRAY addreses for each parameter
        add     ARRAY3, OSUM, ARRAY
        add     ARRAY1, OADDEND1, ARRAY
        add     ARRAY2, OADDEND2, ARRAY

    // INPUT1 = OADDEND1->aulDigits[0]
    // INPUT2 = OADDEND2->aulDigits[0]
    // x1 = OADDEND1->aulDigits[0] + OADDEND2->aulDigits[0]
        ldr     INPUT1, [ARRAY1, LINDEX, lsl 3]
        ldr     INPUT2, [ARRAY2, LINDEX, lsl 3]
        adds    x1, INPUT1, INPUT2
    // OADDEND1->aulDigits[0] = x1
        str     x1, [ARRAY3, LINDEX]

    // if (LSUMLENGTH == 0) go to endloop1
    ...
    // lIndex = 1;
     ...
     // if (LSUMLENGTH == 1) go to endloop1
        eor     x3, LSUMLENGTH, LINDEX
        cbz     x3, endloop1  
    ...
    // INPUT1 = OADDEND1->aulDigits[LINDEX]
    // INPUT2 = OADDEND2->aulDigits[LINDEX]
    // x2 = INPUT1 + INPUT2
        ldr     INPUT1, [ARRAY1, LINDEX, lsl 3]
        ldr     INPUT2, [ARRAY2, LINDEX, lsl 3]
    ...
    // if (LSUMLENGTH != LINDEX) go to loop1
        eor     x3, LSUMLENGTH, LINDEX
        cbnz    x3, loop1
